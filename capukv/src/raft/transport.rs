...
impl RaftPeer {
        ...
        let client = match &*lock {
            None => {
                ...
                let client =
                    ...
                    .await
                    .map_err(|_e| {
                        crate::Error::Transport("Timed-out getting connection".to_string())
                    })??;
    ...
    async fn should_log_failure(&self) -> bool {
        ...
        let backoff_dur = Self::BACKOFFS
            [(Self::BACKOFFS.len() - 1).min(lock.logs_since_success - Self::INITIAL_LOGS)];
    ...
    pub(crate) async fn request_vote(
        &self, args: proto::VoteRequest, msg_tx: mpsc::Sender<RaftMessage>,
    ) {
        let err = match self.get_client().await {
            Ok(mut client) => {
                ...
                match result {
                    Ok(result) => {
                        msg_tx
                            .send(RaftMessage::RequestVoteResponse(result.into_inner()))
                            .await
                            .unwrap();
    ...
    pub(crate) async fn append_entries(
        &self, args: proto::AppendEntriesRequest, msg_tx: mpsc::Sender<RaftMessage>,
    ) {
        tracing::trace!(
        ...
        let err = match self.get_client().await {
            Ok(mut client) => {
                ...
                match result {
                    Ok(result) => {
                        msg_tx
                            .send(RaftMessage::AppendEntriesResponse(result.into_inner()))
                            .await
                            .unwrap();
                        self.logging_meta.lock().await.success();
                        return;
...
impl proto::raft_service_server::RaftService for &'static raft::Raft {
    async fn append_entries(
        ...
        let resp =
            receiver.await.map_err(|e| tonic::Status::internal(format!("RecvError: {:?}", e)))?;
        Ok(resp.into())
    }

    async fn request_vote(
        &self, req: Request<proto::VoteRequest>,
    ) -> Result<Response<proto::VoteResponse>> {
        let (_, _, req) = req.into_parts();
